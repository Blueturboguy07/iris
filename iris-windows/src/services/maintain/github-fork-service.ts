...
import type { MaintainPoolFetchLike } from "./pool-client";
...
/** The device-flow connection state, published to the caller the same way
 *  Swift's `@Published connectState` is — `currentConnectState()` here plus
 *  `connect()`'s `onStateChange` callback stand in for the observation. */
...
/** The access/refresh token pair, read/written by four plain closures — see
 *  the file header for why this never touches `secrets.ts` directly. */
...
export interface GitHubPropagateFixOptions {
  readonly branch: string;
  readonly canonicalRepo: string;
  readonly cloneRunner: MaintainShellRunner;
...
export type GitHubPropagateFixResult =
  | { readonly type: "not_connected" }
  | { readonly type: "failed"; readonly reason: string }
...
/** One `iris/fix-*` PR open on a repo the connected user owns — the shape
 *  `incoming-fix-reviewer.ts`'s `review()` re-verifies before anyone merges. */
export interface IncomingFixPullRequest {
...
/** Whether a `owner/name` slot on GitHub is empty, is our own fork of
 *  `canonicalRepo`, or is someone else's unrelated repo we must never touch. */
type RepoRelationship = "absent" | "is_our_fork" | "unrelated_repo";

/** The bare shape this file needs out of a GitHub API response — status plus
 *  a parsed (and still unvalidated) JSON body. */
interface GitHubApiResponse {
  readonly status: number;
  readonly json: unknown;
}

export interface GitHubForkServiceOptions {
  /** `undefined` = read `process.env.IRIS_GITHUB_APP_CLIENT_ID`; an explicit
   *  `string` or `null` (including in tests) is used as-is. */
  readonly clientId?: string | null;
  readonly tokenStorage: GitHubTokenStorage;
...
export class GitHubForkService {
  ...
  constructor(options: GitHubForkServiceOptions) {
    ...
    this.sleepMs =
      options.sleepMs ?? ((milliseconds: number) => new Promise((resolve) => setTimeout(resolve, milliseconds)));
   ...
   */
  async backUp(options: GitHubBackupOptions): Promise<GitHubBackupResult> {
  ...
  async propagateFix(options: GitHubPropagateFixOptions): Promise<GitHubPropagateFixResult> {
    const accessToken = await this.currentAccessToken();
    if (accessToken === null) {
      return { type: "not_connected" };
    }
   ...
   * normal GitHub flow.
   */
  async incomingFixPullRequests(canonicalRepo: string): Promise<IncomingFixPullRequest[]> {
    ...
    const pullRequests: IncomingFixPullRequest[] = [];
   ...
   */
  async mergeIncomingFixPR(pr: IncomingFixPullRequest): Promise<boolean> {
    const token = await this.currentAccessToken();
    if (token === null) {
      return false;
    }
  ...
  private async pushBranch(options: {
    ...
    const command = `git push ${singleQuoteForShell(options.pushUrl)} ${singleQuoteForShell(
...
function splitOwnerRepo(canonicalRepo: string): [string, string] {
  const parts = canonicalRepo.split("/");
