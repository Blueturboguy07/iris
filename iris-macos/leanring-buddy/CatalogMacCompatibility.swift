...
nonisolated enum CatalogMacCompatibility: String, Equatable, Sendable, Decodable {
    case desktopApp
    case localWebApp
    case mobileOnly
    ...
    /// Resolve a guide response without discarding trusted catalog metadata.
    /// Transport, decoding, slug, and unsupported-content misses are not
    /// evidence that an explicitly classified catalog app became unknown.
    static func resolved(
        from data: Data?, expectedSlug: String, fallback: Self
    ) -> Self {
        guard let data else { return fallback }
        let published = fromPublishedGuideData(data, expectedSlug: expectedSlug)
        return published == .unknown ? fallback : published
    }
}
