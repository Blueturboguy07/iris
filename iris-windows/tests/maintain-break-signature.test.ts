...
describe("normalizeMessage parity with publik's normalizeBreakMessage", () => {
  ...
  const publikOutputForVector: ReadonlyArray<readonly [string, string]> = [
    ["", ""],
  ...
  it.each(publikOutputForVector)("normalizes %j the way publik does", (rawMessage, expected) => {
    expect(normalizeMessage(rawMessage)).toBe(expected);
  });
});
