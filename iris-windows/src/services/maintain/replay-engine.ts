...
import { verifyAppliedPatch, earnsCleanApply } from "./verification-harness";
...
import { makeChatRequest, ANTHROPIC_API_VERSION } from "../assistant-transport";
import type { AssistantTransport } from "../assistant-transport";
...
function decodeVersionRanges(value: unknown): VersionRange[] | undefined {
  if (!Array.isArray(value)) return undefined;
...
/** What `recipeApplicabilityMatches` needs to decide one recipe against one
 *  machine. `applicability` is the recipe's opaque wire value (see below);
 *  `appVersion` is `null`, never `undefined`, when the machine's installed
 *  version could not be determined — matching Swift's `String?` exactly. */
export interface RecipeApplicabilityMatchOptions {
 ...
 */
export function recipeApplicabilityMatches(options: RecipeApplicabilityMatchOptions): boolean {
...
/** The forced tool-call schema. Deliberately narrow — the model may return
 *  the adapted diff, or decline via `cannot_adapt`; nothing else. */
...
const ADAPT_PATCH_TOOL: AnthropicToolDefinition = {
  name: "adapt_patch",
...
/** What the caller passes in for one adapt attempt — mirrors Swift's
 *  `adaptPatch(diagnosis:stalePatch:localFileExcerpts:appSlug:)` parameters,
 *  bundled per this codebase's options-object convention. */
export interface AdaptPatchOptions {
...
/** Mirrors Swift's `MaintainFixAdapting` protocol. `RecipeReplayEngine`'s
 *  `fixAdapter` field is typed against this, not the concrete
 *  `AnthropicPatchAdapter`, so a test can substitute a scripted fake. */
export interface MaintainFixAdapting {
  adaptPatch(options: AdaptPatchOptions): Promise<MaintainFixAdaptation>;
}
...
interface AnthropicMessagesResponseBodyWithTools {
  readonly content?: ReadonlyArray<AnthropicToolUseContentBlock>;
}
...
export class AnthropicPatchAdapter implements MaintainFixAdapting {
  ...
  async adaptPatch(options: AdaptPatchOptions): Promise<MaintainFixAdaptation> {
    ...
    let parsedResponseBody: AnthropicMessagesResponseBodyWithTools;
    try {
      parsedResponseBody = JSON.parse(rawResponseBody) as AnthropicMessagesResponseBodyWithTools;
    } catch {
 ...
 *  Mirrors Swift's `RecipeReplayResult` enum, one case each. */
...
/** Everything `RecipeReplayEngine.replay` needs for one attempt, bundled per
 *  this codebase's options-object convention (matches `AutopilotRunner`'s
 *  method shapes rather than Swift's positional parameters). */
export interface RecipeReplayOptions {
  readonly recipe: PooledFixRecipe;
  readonly appSlug: string;
  readonly appStack: BreakAppStack;
  /** `null`, never `undefined`, when the installed version could not be
   *  determined — matches `RecipeApplicabilityMatchOptions.appVersion`. */
  readonly installedAppVersion: string | null;
  readonly signatureId: string;
  readonly machineArchitecture: string;
}

/** Constructor collaborators for `RecipeReplayEngine` — see the CROSS-MODULE
 *  CONTRACT note above for why `getCurrentInstallId` and `createShellRunner`
 *  are injected functions rather than concrete-class imports. */
export interface RecipeReplayEngineOptions {
  readonly provenanceStore: InstallProvenanceStore;
  readonly poolClient: MaintainPoolClient;
  /** Reads the current pseudonymous install id — the real implementation is
   *  `MaintainInstallIdentity.currentInstallId` bound by the caller. */
  readonly getCurrentInstallId: () => string;
  readonly patchQueue: PatchQueue;
  /** Builds a shell runner rooted at `repoRootPath`, or `undefined` when one
   *  cannot be constructed (mirrors Swift's `try? MaintainShellRunner(repoRootPath:)`). */
...
interface ApplyVerifyAndCommitOptions {
...
export class RecipeReplayEngine {
  ...
  constructor(options: RecipeReplayEngineOptions) {
  ...
  async replay(options: RecipeReplayOptions): Promise<RecipeReplayResult> {
   ...
   */
  private async applyVerifyAndCommit(options: ApplyVerifyAndCommitOptions): Promise<RecipeReplayResult> {
    ...
    try {
      ...
      const dryRun = await tryRun(runner, `git apply --check --3way ${powerShellSingleQuote(patchFileName)}`, {
        deadlineMs: 60_000,
      });
