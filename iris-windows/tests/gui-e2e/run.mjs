...
async function scenarioAutopilot(exePath) {
  ...
  // The one-time "Let Iris take control?" consent is auto-granted here via the
  // IRIS_E2E flag in `baseEnv` (see `ensureAutonomyGranted` in main/index.ts) —
  // otherwise the autopilot's first run blocks on a modal no one can click.
