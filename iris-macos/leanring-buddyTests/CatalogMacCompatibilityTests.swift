...
struct CatalogMacCompatibilityTests {
    ...
    @Test func guideSlugMappingAndExplicitCompatibilitySurviveGuideMisses() throws {
        let descriptorData = Data("""
        {"slug":"catalog-name","name":"Fixture","macBundleId":null,
         "latestReleaseTag":"v1","guideSlug":"published-route",
         "macCompatibility":"localWebApp"}
        """.utf8)
        let descriptor = try JSONDecoder().decode(CatalogAppDescriptor.self, from: descriptorData)

        #expect(descriptor.slug == "catalog-name")
        #expect(descriptor.guideSlug == "published-route")
        #expect(descriptor.macCompatibility == .localWebApp)
        #expect(CatalogMacCompatibility.resolved(
            from: nil, expectedSlug: descriptor.guideSlug!, fallback: descriptor.macCompatibility
        ) == .localWebApp)
        #expect(CatalogMacCompatibility.resolved(
            from: Data("not json".utf8), expectedSlug: descriptor.guideSlug!,
            fallback: descriptor.macCompatibility
        ) == .localWebApp)
    }

    @Test func validGuideUsesPublishedSlugInsteadOfCatalogSlug() throws {
        let data = try guideData(slug: "published-route", outputType: "desktop_app")
        #expect(CatalogMacCompatibility.resolved(
            from: data, expectedSlug: "published-route", fallback: .unknown
        ) == .desktopApp)
        #expect(CatalogMacCompatibility.resolved(
            from: data, expectedSlug: "catalog-name", fallback: .localWebApp
        ) == .localWebApp)
    }
