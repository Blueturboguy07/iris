...
import type { IncomingFixPR } from "./github-fork-service";
...
import { earnsCleanApply, verifyAppliedPatch, type VerificationCommands } from "./verification-harness";
...
export interface IncomingFixReview {
  readonly pr: IncomingFixPR;
  /** True only when the fix rebuilt and passed the full suite on the owner's
   *  own machine. A clean merge is never offered without this. */
...
// ---------------------------------------------------------------------------
// The local stand-in for `verification-commands.ts` (see the file header)
// ---------------------------------------------------------------------------

/** The file-existence/`package.json`-script checks
 *  `defaultVerificationCommandsForStack` needs, as an injected seam so the
 *  per-stack table is testable without a real clone on disk — mirrors
 *  `install-provenance.ts`'s injected `checkGitDirectoryExists`. */
...
export class IncomingFixReviewer {
  ...
  constructor(options: {
    ...
    /** Builds a `MaintainShellRunner` rooted at an arbitrary path.
     *  `services/` cannot spawn a real one itself (no `child_process`) — the
     *  real factory (a one-shot `powershell.exe -EncodedCommand` per call)
     *  lives in `main/maintain/`, wired in there. Called twice per review:
     *  once rooted at the clone to fetch and create the worktree, once
     *  rooted at the worktree itself to build/test it. */
    readonly createShellRunner: (repoRootPath: string) => MaintainShellRunner;
    /** Defaults to the local stand-in above. */
   ...
   */
  async review(pr: IncomingFixPR, appSlug: string, appStack: BreakAppStack): Promise<IncomingFixReview> {
