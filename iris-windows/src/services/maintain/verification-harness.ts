...
/// The per-stack command vocabulary a fix is verified against. Code-authored,
/// never model-authored — which is why these strings never pass through
/// `autopilot/risk.ts`'s gate. Kept in this file, not a sibling module, for the
/// same reason Swift keeps `VerificationCommands` inside `VerificationHarness.swift`:
/// the two are one concept, the command table and the thing that runs it.
export interface VerificationCommands {
  readonly buildCommand?: string;
  readonly testCommand?: string;
  /// Where inside the repo the build/test commands run (Tauri keeps its JS in
  /// `ui/`, its Rust at the root). Absent runs at the repo root.
  readonly commandSubdirectory?: string;
}

/// What one verification run proved. Serialized into the recipe pointer's
/// `verification` jsonb and into the commit trailer block by whichever module
/// records the outcome — this type is the wire shape both sides agree on.
export interface VerificationOutcome {
  readonly reproFailedBeforePatch?: boolean;
  readonly reproPassedAfterPatch?: boolean;
  readonly reproFailedOnRevert?: boolean;
  readonly buildSucceeded: boolean;
  readonly suitePassed?: boolean;
  /// The failing stage's output tail when the gate blocked, for the diagnosis
  /// record — never shown raw to the user.
  readonly blockedStage?: string;
  readonly blockedOutputTail?: string;
}
...
async function enforceDiffScope(runner: MaintainShellRunner): Promise<DiffScopeVerdict> {
