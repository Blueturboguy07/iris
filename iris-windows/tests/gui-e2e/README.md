...
|---|---|---|
| **A · Core UI** | no-credential onboarding (`#byo-key`, publik API setup, the Settings gear); Anthropic+OpenAI key set/clear matrix incl. empty-string-clears and key independence; Settings window opened from the chat's gear (OpenAI field renders; **no horizontal overflow at the window's narrowest width**, real Segoe UI); signed-out `sendQuery` prompts for a key instead of making a paid call; guide window opens; overlay present; `iris://` deep-link **accept + unknown-param reject**; **closing the chat then launching Iris again reopens the chat** (and a further launch keeps one chat) | safeStorage/**DPAPI** round-trip (`secretStorageAvailable === true`, encrypt/decrypt); real `iris://` delivery and a real plain relaunch via the **single-instance lock** (second launch → `second-instance` → parser or chat) |
