...
**The Eye Is A Control**: The eye is 64pt and clicking it opens a minimal input bar under it — one field, a few suggestion chips — while the eye itself becomes a gear that opens the settings panel. Two rules make that safe on a window that covers the whole screen:
...
| `OverlayEyeInteraction.swift` | ~287 | The eye as a *control*, with no AppKit or SwiftUI in it so all of it is testable without a screen: `OverlayEyeInteractionGeometry` (the 64pt eye, its resting place, the one rectangle that may accept a click, and where the input bar hangs), `OverlayEyeActivation` (eye → bar → gear → settings), and `OverlayEyeSuggestions` — the suggestion chip strings, in one place, guide-aware. |
| `OverlayEyeInputBar.swift` | ~282 | The minimal bar that opens under the eye: one field, a few chips, nothing else. Its own `.nonactivatingPanel` so it can take keystrokes without activating Iris and without the full-screen overlay ever needing to become key. Sends through `CompanionManager.sendUserMessage` — no second pipeline. |
