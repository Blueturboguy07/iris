...
| `OnDemandEditCoordinator.swift` | ~1042 | The user-initiated on-demand edit phase machine (pick → describe → consent → run → preview keep/discard → relaunch → done), all safety rails live-checked; injected seams for the engine call, fork backup, packaging, relaunch, and public publish. Also `OnDemandEditScopeEstimate`, the pure up-front "too large for one edit" refusal. |
