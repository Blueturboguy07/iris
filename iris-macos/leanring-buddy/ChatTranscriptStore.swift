...
//  Two strings and a date for each exchange. A non-text reset marker separates
//  the current conversation from the archive without deleting either one.
//
...
/// worth having, but the bar only reopens on the current conversation's last
/// exchange. There
...
final class ChatTranscriptStore {
    ...
    /// The archive remains intact when New chat starts. A marker in the same
    /// atomic file records where the currently resumable conversation begins.
    ...
    /// The bar and model resume only exchanges after the last New chat marker.
    /// `recentExchanges` still includes all archived conversations for History.
    ...
    /// The marker persists even if Iris quits before another question is sent.
